...
function populateTable(dataToDisplay) {
    ...
    dataToDisplay.forEach((row, index) => {
        ...
        tr.innerHTML = `
            ...
            <td class="px-4 py-3.5 font-medium text-gray-600" data-label="2nd Wash Qty">${row['2nd Wash Qty'] || 0}</td>
            <td class="px-4 py-3.5">
                <button class="open-batch-modal-btn bg-gray-700 hover:bg-blue-800 text-white px-6 py-2 rounded-lg text-sm font-semibold tracking-wide transition-all duration-200 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5">
                    2nd Wash Batch
                </button>
            </td>
...
const workOrderData = {}; // Stores batch numbers and 2nd wash quantities for each work order
...
saveBatchDataBtn.addEventListener('click', () => {
    ...
    const firstWashQtyCell = currentActiveRow.querySelector('[data-label="2nd Wash Qty"]');
    ...
    if (isMergedRow) {
        // If it's a merged row, find it in currentTableState and update its 2nd Wash Qty
        ...
        if (mergedRowIndex !== -1) {
            window.currentTableState[mergedRowIndex]['2nd Wash Qty'] = currentFirstWashQty;
        }
    } else {
        // If it's an original row, find it by its _originalIndex and update its 2nd Wash Qty
        ...
        if (stateIndex !== -1) {
            window.currentTableState[stateIndex]['2nd Wash Qty'] = currentFirstWashQty;
        }
    }
...
function mergeSelectedRows() {
    ...
    selectedDataObjects.forEach((dataRow, index) => {
        ...
        mergedFirstWashQty += (dataRow['2nd Wash Qty'] || 0);
    ...
    const newMergedData = {
        ...
        '2nd Wash Qty': mergedFirstWashQty // The summed 2nd wash quantity
    };
